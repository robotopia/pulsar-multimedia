...
% single-pulse polarisation data of B0809+74, as judged by the
% description in Manchester et al (1975).

npulses = 50;
dr      = -1.0; % drift rate (degrees per pulse)
phases  = [-30:0.25:30]; % in degrees
nphases = length(phases);
pulses  = [1:npulses];
spw     = 1.2; % FWHM of subpulses in degrees
sps     = 10; % P2 (degrees)
...
% Function that translates "subpulse phase"     (-0.5 to   0.5)
%                      and "pulse phase"        (-0.5 to   0.5)
%                       to "polarisation angle" ( 0   to 180  )
% (This function is designed to emulate B0809+74)
function res = sp2pa(sp, pp)
    ph = mod(sp+0.5, 1)-0.5;
    transitionpoint = -pp * 10;
    transitionrate  = 30;
...
PolPA   = sp2pa(SPPhase ./ sps, PHS / 360);
